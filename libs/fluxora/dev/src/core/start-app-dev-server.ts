...
import { resolveRoutes } from "@fluxora/router/node";
import { appCtx } from "@fluxora/runtime";
import type { RegisteredApp } from "@fluxora/types";
import { FLUXORA_APP_CONTAINER, VIRTUAL_ENTRIES } from "@fluxora/utils";
...
export const startAppDevServer = async (app: RegisteredApp) => {
  ...
  adapterContext.server.use(async (req, res) => {
    ...
    if (!appModule.config.ssr) tags.push({ tag: "div", attrs: { id: FLUXORA_APP_CONTAINER }, injectTo: "body" });
